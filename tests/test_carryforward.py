...
def _put(conn, period, iid, status="outstanding", periods_open=1, first="2024-06",
    ...
    conn.execute(
        ...
        "direction,category,amount,source_ref,status,first_seen_period,periods_open,resolution,"
        "resolved_by,evidence_json) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        (iid, "R" + period, "E", "A", period, "ledger_outstanding", "inflow",
         "deposit_in_transit", "500.00", "ledger row 1", status, first, periods_open,
...
def test_item_clears_next_period_auto_resolved(memdb):
    # June item outstanding; July run does not re-derive it (matched) => auto-resolved
    _put(memdb, "2024-06", "ITEM1", status="outstanding")
    anoms, i8 = finalize_carryforward(memdb, "E", "2024-07", current_ids=set())
    row = memdb.execute("SELECT status, resolved_by FROM reconciling_items WHERE id='ITEM1'").fetchone()
    assert row["status"] == "resolved" and row["resolved_by"] == "auto_carryforward"
...
def test_stale_after_three_periods(memdb):
    ...
    anoms, i8 = finalize_carryforward(memdb, "E", "2024-08", {"ITEM3"})
